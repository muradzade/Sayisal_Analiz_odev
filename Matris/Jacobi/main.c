...
double *jacobi(double **katsayilar,int N,double E);
...
int main(int argc, char *argv[]) {
	...
	double *degiskenler=jacobi(katsayilar,N,E);
...
double *jacobi(double **katsayilar,int N,double E)
{
	...
	while(flag)
	{
		for(i=0;i<N;i++) onceki_degiskenler[i]=degiskenler[i];
		
		for(i=0;i<N;i++)
		{
			degiskenler[i]=f(katsayilar[i],N,onceki_degiskenler,i);
		}
